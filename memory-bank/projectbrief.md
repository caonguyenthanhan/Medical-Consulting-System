...
- Bổ sung tính năng Trò chuyện bằng giọng nói và thị giác (Vision): người dùng có thể chụp/tải ảnh thuốc hoặc thông tin y khoa và đặt câu hỏi bằng giọng nói; hệ thống phân tích ảnh bằng VLM và trả lời, đồng thời phát âm thanh tự động.
- Frontend thêm camera UI tại `app/speech-chat/page.tsx`, hỗ trợ chụp ảnh, upload, preview và gửi kèm tin nhắn.
- Backend bổ sung endpoint `POST /v1/vision-chat` cho text + image (VLM) và `POST /v1/speech-chat-optimized` cho pipeline giọng nói tối ưu (chunking STT + streaming TTS).
- Mô hình sử dụng: LLM văn bản (Llama-3.2-1B-Instruct-Q6_K_L) và VLM (llava-v1.5-7b-Q6_K với mmproj), nạp thành công khi khởi động server (`server.py`).